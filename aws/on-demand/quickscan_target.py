...
# flake8: noqa
"""Scan an AWS S3 bucket with the CrowdStrike Quick Scan Pro API.

Scans an AWS S3 bucket using the CrowdStrike Falcon Quick Scan and Sample Uploads APIs.
...
- Memory-efficient streaming of files directly from AWS S3
...
- Recommended deployment pattern involves running in AWS within a container, an EC2 instance
or as a serverless lambda.
...
- Region: AWS Region containing target bucket
...
python3 quickscan_target.py -r REGION -t s3://BUCKET_NAME -k API_KEY -s API_SECRET [-b BATCH_SIZE] [-w WORKERS] [-d CHECK_DELAY] [-l LOG_LEVEL]

Example scenarios:
1. Process 1500 files in batches of 500 using 10 workers:
   python3 quickscan_target.py -r REGION -t s3://BUCKET_NAME -k API_KEY -s API_SECRET -b 500 -w 10

2. Default processing (1000 file batches, 10 workers):
   python3 quickscan_target.py -r REGION -t s3://BUCKET_NAME -k API_KEY -s API_SECRET
"""
...
class QuickScanApp:
    ...
    def run(self):
        ...
        try:
            if self.config.bucket:
                ...
                raise SystemExit(
                    "Invalid bucket name specified. Please include 's3://' in your target."
                )
    ...
    def upload_bucket_samples(self):
        ...
        s_3 = boto3.resource("s3", region_name=self.config.region)
        try:
            bucket = s_3.Bucket(self.config.target_dir)
